test_that("obs_only", {
  newdata <- expand.grid(Tree = unique(Orange$Tree), age = c(1, 2, 3, 4, 5),
...
test_that("new_data obs_only", {
  data <- expand.grid(Fac1 = c("1", "2", "4"), Fac2 = c("1", "3"),
  ...
  expect_identical(nrow(new_data(data, c("Fac1", "Fac2", "Fac3"),
                                 obs_only = TRUE)), 10L)
  expect_identical(nrow(new_data(data, c("Random"), obs_only = TRUE)), 2L)
  expect_identical(nrow(new_data(data, c("Fac3", "Random"))), 6L)
  expect_identical(nrow(new_data(data, c("Fac3", "Random"), obs_only = TRUE)),
                   4L)
  expect_identical(new_data(data, c("Fac1", "Fac2", "Fac3"), obs_only = TRUE),
                   new_data(data, c("Fac1", "Fac2", "Fac3"),
                            obs_only = list(c("Fac1", "Fac2", "Fac3"))))
  expect_equal(nrow(new_data(data, c("Fac1", "Fac2", "Fac3", "Random"),
                             obs_only = TRUE)), 20L)
  expect_identical(nrow(new_data(data, c("Fac1", "Fac2", "Fac3", "Random"),
                                 obs_only = list("Fac1", "Fac2", "Fac3",
                                                 "Random"))), 24L)
})
