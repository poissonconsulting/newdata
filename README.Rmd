...
New values can be cast to the class of the original variable with `xcast()`.
```{r}
...
`new_data()` is a superseded wrapper on `xnew_data()` that is provided for consistency with existing code.
It takes the variables to vary as a character vector and the length of all the sequences as a count.
